        ...
        carOwner.getCarOwnerByUserId(userInfo._id).then(carOwnerInfo => {
            store.getStore(storeId)
                .then(storeInfo => {
                            ...
                            if (warehouseInfo.storage[0].amount >= quantity) {
                                ...
                                if (totalPrice > 0) {
                                    cartItem.createCartItem({
                                        ...
                                        return res.status(400).send({error: 'Invalid cartItem data'});
                                    });
                                } else {
                                    return res.status(400).send({error:'error With The Price'});
                                }
                            } else {
                                return res.status(400).send({error:'error This Quantity Is not available'});
                            }
                        }).catch(err => {
                    ...
                    return res.status(404).send({error:'store id not found'});
                });
        }).catch(err => {
            return res.status(400).send({error:'UNAUTHORIZED_USER'});
        });
    } else {
                        ...
                        await cartItem.getCartItem(item).then(async cart => {
                            await store.getStore(cart.storeId).then(async retrivedStore => {
                                await warehouse.getProductFromWarehouse(retrivedStore.warehouse, cart.product).then(async retrivedStorage => {
                                            ...
                                            await product.getProductById(cart.product).then(async retrivedProduct => {
                                                ...
                                                await product.updateProduct(retrivedProduct[0]).then(async updatedProduct => {
                                                    await warehouse.decreaseAmaountOfProduct(retrivedStore.warehouse, cart.product, cart.quantity).then(async updatedWarehouse => {
                                                        ...
                                                        res.status(400).send({error:'There is no available quantity in this store'});
                                                    });
                                                }).catch(err => {
                                                    res.status(400).send({
                                    ...
                                    return res.status(404).send({error: 'There no available quantity in this store'});
                                });

                            }).catch(err => {
                                return res.status(404).send({error:'There is no store'});
                            });

                        }).catch(err => {
                            res.status(404).send({error: 'cartItem is not found'});
                        });
                    });
                } else {
                    res.status(400).send({
